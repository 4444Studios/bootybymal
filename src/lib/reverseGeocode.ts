 ...
 * Primary: Photon (komoot). Fallback: OSM Nominatim.
 */
...
function placeFromPhotonProps(props: PhotonProperties): string | undefined {
  return props.city || props.locality || props.name || props.district || props.county
}
...
async function reverseGeocodePhoton(lat: number, lon: number): Promise<string> {
  ...
  return formatPlaceLine(placeFromPhotonProps(props), props.state || props.country)
}
...
async function reverseGeocodeNominatim(lat: number, lon: number): Promise<string> {
  ...
  const res = await fetch(url.toString(), {
    headers: {
      ...
      'User-Agent': 'BootyByMal-Website/1.0 (contact form location)',
    },
  })
  ...
  return formatPlaceLine(place, a.state || a.country)
}
